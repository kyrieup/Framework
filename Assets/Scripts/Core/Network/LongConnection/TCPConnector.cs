...
class TCPConnector
{
    ...
    public bool Connect(String dnsHost, int port, IProtocolResolver protocol_resolver, TCPSession tcpSession = null)
    {
        ...
        this.mSocket = new Socket(this.mEndPoint.AddressFamily, SocketType.Stream, ProtocolType.Tcp);
        if (tcpSession == null)
        {
            tcpSession = new TCPSession();
        }

        this.mSocket.ReceiveTimeout = 10000;
        this.mSocket.SendTimeout = 10000;
